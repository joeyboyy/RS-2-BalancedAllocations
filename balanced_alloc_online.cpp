/*! @file balanced_alloc_online.cpp
* @brief Balanced allocation experiments, online version
...
int main()
{
    ...
    for (int m : ms) // different values of m
    {
        ...
        {
        #ifdef PARTIAL_INFO
            std::string filename = "../out/avgap_online_partial_m=" + std::to_string(m) + "_scheme=" + std::to_string(ks[scheme][0]) + "questions-" + std::to_string(ks[scheme][1]) + "bins.txt";
        #else
            std::string filename = "../out/avgap_online_m=" + std::to_string(m) + "_scheme=" + std::to_string(ds[scheme]) + "_beta=" + std::to_string(betas[scheme]) + ".txt";
        #endif
            std::ofstream out_1(filename);
            if (!out_1) throw;

            std::vector<float> gap(n_max+1, 0); // accumulated gap: gap(n) = sum_{t <= T} G_n
            ...
            for (int t = 0; t < T; t++) // T trials
            {
                ...
                for (int n = 1; n <= n_max; n++) // balls, 1-indexed!
                {
                    // drop ball / determine bin
                #ifdef PARTIAL_INFO
                    int i = drop_ball(uniform_distr, bins, m, ks[scheme][1], ks[scheme][0]);
                #else
                    int i = drop_ball(uniform_distr, bins, ds[scheme], betas[scheme]);
                #endif
                    // place ball
                    int& X_i = bins[i];
                    X_i = X_i + 1;
